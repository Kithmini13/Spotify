...
client_id = 'e15c27940b8345d985a0757fed73ef72'
client_secret = '11dceb81586340349daa2dd690d45876'
...
# Replace 'artist_name' with 'artist(s)_name' wherever it's used
for i, row in df_spotify.iterrows():
    # try:
        # print(f"Processing: {row['track_name']} by {row['artist(s)_name']}")
        track_id = search_track(row['track_name'], row['artist(s)_name'], access_token)
        if track_id:
            image_url = get_track_details(track_id, access_token)
            df_spotify.at[i, 'image_url'] = image_url
    #     else:
    #         print(f"Track not found: {row['track_name']} by {row['artist(s)_name']}")
